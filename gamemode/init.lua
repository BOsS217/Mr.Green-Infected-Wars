...
AddCSLuaFile("cl_xrayvision.lua")
...
include("sv_anti_map_exploit.lua")
include("sv_savedata.lua")

include("greencoins/sv_greencoins.lua")
...
function GM:Initialize( )
	...
	for _, filemap in pairs( listOfResourceMaps ) do
		for _, filename in pairs(file.Find(filemap.."/*.*", "GAME")) do
	...
	for k, wildcard in pairs(listOfWeaponModels) do
		for _, filename in pairs(file.Find("models/weapons/*"..wildcard..".*", "GAME")) do
			resource.AddFile("models/weapons/"..filename)
		end
		for _, filename in pairs(file.Find("models/weapons/infectedwars/*"..wildcard..".*", "GAME")) do
			resource.AddFile("models/weapons/infectedwars/"..filename)
		end
	end

	resource.AddFile("resource/fonts/doom.ttf")
	resource.AddFile("resource/fonts/BIRDMAN_.TTF")
	resource.AddFile("materials/mrgreen/mrgreen.vmt")
	resource.AddFile("materials/mrgreen/mrgreen.vtf")
	...
	resource.AddFile("scripts/sounds/infectedwars_sweps_fire2.txt")
	
	--Create data folders when they don't exist
	if not file.IsDir("infectedwars", "DATA") then
		file.CreateDir("infectedwars")
	end 
	if not file.IsDir("infectedwars/maps", "DATA") then
		file.CreateDir("infectedwars/maps", "DATA")
	end
...
function GM:InitializeVars()
	...
	timer.Create("FirstSlay",60+math.random(10),1,function()
		self:StartGameKill()
	end)
	timer.Create("FirstSlayBackup",200,1,function()
		self:StartGameKill()
	end) -- second one in case there are still no undead
end
...
function GM:StartGameKill()
	...
	if TESTMODE and #pltab == 1 then
		return
	end

	self.MaxReinforcements = self.Reinforcements or 300
	
	local humans = team.NumPlayers(TEAM_HUMAN)
	if humans <= 2 then
		timer.Destroy("FirstSlay")
		timer.Create("FirstSlay",15,1,function()
			self:StartGameKill()
		end)
		return
	end
	
	local required = 5-team.NumPlayers(TEAM_UNDEAD)+CountStartTeams(TEAM_UNDEAD)
	if humans <= 23 then
		required = required-1
		if humans <= 14 then
			required = required-1
...
function GM:PlayerInitialSpawn( pl )
	...
	pl.MarkedThisLife = {}
	pl.BabiesKilled = 0
	pl.KilledAsStalker = 0
	pl.BullshotKills = 0
	pl.ScreamEffect = 0
	pl.Blasted = {}
	pl.BabyMurdered = {K = false,G = false,B = false}
...
function GM:SynchronizeReinforcements( pl )
	umsg.Start("SendReinforce", pl)
		...
		umsg.Short(self.MaxReinforcements)
	umsg.End()
end
...
function GM:PlayerSpawn( pl )
	...
	if (pl.FirstSpawn and not ENROUND) then
		...
		--TODO: Simple timer?
		timer.Create(pl:UniqueID().."menutimer",0.05,0,function()
			if pl.Validated then
				pl:ConCommand(com)
				timer.Destroy(pl:UniqueID().."menutimer")
			end
		end)
	else		
		-- Normal spawning
		...
		// SUIT TESTING
		for k, v in pairs(suitsData1) do
...
function GM:Think( )
	...
	if (self.GameState == ROUNDSTATE_PLAY and not ENDROUND) then
		if (team.NumPlayers(TEAM_HUMAN) <= 0 and not TESTMODE) then
			if self:RoundTimeLeft() > ROUNDLENGTH/2 and #player.GetAll() >= 10 then
				for k, pl in pairs(team.GetPlayers(TEAM_UNDEAD)) do
					pl:UnlockAchievement("steamroller")
				end
				if self:RoundTimeLeft() > ROUNDLENGTH*0.75 then
					for k, pl in pairs(team.GetPlayers(TEAM_UNDEAD)) do
...
function GM:EndRound( winner, reason )
	...
	-- Send statistics to Left4Green
	params = "&add_total_rounds=1"
	params = params.."&add_mapplayed_"..game.GetMap().."=1"
	params = params.."&add_totalkills="..Stats.TotalKills
	params = params.."&add_totaldeaths="..Stats.TotalDeaths
	
	if (winner == TEAM_UNDEAD) then
		params = params.."&add_undead_wins_"..game.GetMap().."=1"
	elseif (winner == TEAM_HUMAN) then
		params = params.."&add_human_wins_"..game.GetMap().."=1"
	end
	if (reason) then
		params = params.."&add_winreason_"..reason.."=1"
	end
	for weapon, value in pairs(Stats.WeaponKills) do
		params = params.."&add_weapon_"..weapon.."="..value
	end
	
	local statupdate = "http://www.mr-green.nl/portal/statistics/rec_stats.php?game=iw"..params
	//print("Sending http stat update: "..statupdate)
	http.Get( statupdate, "", function(a,b) end )
...
function GM:DoPlayerDeath( ply, attacker, dmginfo )
	...
	ply.MarkedThisLife = {}
	ply.BabyMurdered = {K = false,G = false,B = false}
	ply.KilledAsStalker = 0
	ply.BullshotKills = 0
	...
	if (( dmginfo:GetDamage() > 50 or deadeffort ) and not ply.Dissolving ) then
		...
		if deadeffort then
			...
			if IsValid(ent) then
				...
				ent.Name = function() return "< Behemoth Kamikazi >" end
				ent:Activate()
	...
	if ( attacker:IsValid() && attacker:IsPlayer() ) then
		...
		if ( attacker:Team() ~= ply:Team()) then
			...
			if inf == "iw_knife" and ply:Team() == TEAM_UNDEAD and ply:GetPlayerClass() == CLASS_Z_STALKER and (attacker.ScreamEffect or 0) > CurTime() then
				attacker:UnlockAchievement("screamingirony")
			end
			...
			if (dmginfo:GetInflictor():GetClass() == "turret") then -- turret be killing mah enemies!
				...
				if attacker:GetScore("turretkills") >= 100 then
					...
					if attacker:GetScore("turretkills") >= 2029 then
						attacker:UnlockAchievement("terminator")
					end
				end
			end
			...
			if (attacker:Team() == TEAM_UNDEAD) then
				...
				if ply.IsUsingScope then
					attacker:UnlockAchievement("noscopezone")
				end
				...
				elseif attacker:GetPlayerClass() == CLASS_Z_STALKER then -- Stalker 
					...
					attacker.KilledAsStalker = attacker.KilledAsStalker + 1
					if attacker.KilledAsStalker >= 3 then
						attacker:UnlockAchievement("jacktheripper")
					end
					if ply:GetPlayerClass() == CLASS_H_EXPERIMENTAL then
						if IsValid(ply.Turret) then
							attacker:UnlockAchievement("heartbreaker")
						end
					end
				elseif attacker:GetPlayerClass() == CLASS_Z_BONES then -- Bones
					...
					if inf and string.find(inf,"bullshot") then
						attacker.BullshotKills = attacker.BullshotKills + 1
						if attacker.BullshotKills >= 2 then
							attacker:UnlockAchievement("bonecracker")
						end
					end
				elseif attacker:GetPlayerClass() == CLASS_Z_ZOMBIE then
					if inf and string.find(inf,"bow") then
						if dmginfo:GetDamage() >= 50 then //hope its a headshot
							attacker:UnlockAchievement("headache")
						end
					end
				end
	...
	if (ply:Team() ~= TEAM_UNDEAD) then
		...
		if (ply:GetPlayerClass() == CLASS_Z_BEHEMOTH or ply.SpawnAsClass == CLASS_Z_BEHEMOTH) then
			ply.BeheDeaths = (ply.BeheDeaths or 0) + 1
		end
	end
...
function SwitchToUndead( ply )
	...
	timer.Simple(0.5,function(pl) -- send with delay (gives client time to apply teamswitch)
		if ENDROUND or not IsValid(pl) then return end
		pl:ConCommand("iw_menu_class")
	end,ply)
end
...
function GM:EntityTakeDamage( ent, inflictor, attacker, amount, dmginfo )
	...
	if ent:IsPlayer() then
		...
		//testing anti-armor bug
		if dmginfo:GetDamageType() == DMG_BLAST and ply:Team() == TEAM_HUMAN then
			if (ply.NextBlastDmg or 0) > CurTime() then
				--print("TWICE THE POWER!")
				dmginfo:SetDamage(math.min(35,ply:Health()))
				--return
			else
				ply.NextBlastDmg = CurTime() + 0.1
			end
		end
		...
		//print("Damage: "..dmginfo:GetDamage().." by "..tostring(dmginfo:GetAttacker()).." with "..tostring(dmginfo:GetInflictor()).." via "..dmginfo:GetDamageType())
		-- Dynamic fall damage
		if dmginfo:GetDamageType() == DMG_FALL then--if dmginfo:IsFallDamage() then
			dmginfo:SetDamage( 0 )
			...
			if (ply:GetPower() == 0 and ply:SuitPower() > 0 and ply:Team() == TEAM_HUMAN) then
				if (dmginfo:GetDamageType() == DMG_BLAST or dmginfo:IsExplosionDamage()) or attacker:IsPlayer() or attacker:IsNPC() or string.find(attacker:GetClass(),"prop_physics") then //or (dmginfo and (dmginfo:GetDamageType() == DMG_BLAST or dmginfo:IsExplosionDamage())) or (string.find(attacker:GetClass(),"prop_physics") or string.find(attacker:GetClass(),"explosion")) then	
					...
					dmginfo:SetDamage(0)
					ply:SetSuitPower( ply:SuitPower()-(dmg*cost*multiplier) )
		...
		if dmginfo:GetDamage() ~= 0 then
			...
			if attach then
				...
				if headshot and ply.LastHeadShot < CurTime()-1 and GORE_MOD then
					...
					//util.Blood(attach.Pos, math.Rand(3,5), (VectorRand() + Vector(0,0,math.random(0,1))):GetNormal() , vel:Length() * 0.75,true)
				end
			end
...
function StompShockWave( ply )
	...
	for k, ent in pairs(ents.FindInSphere(pos,250)) do
		if ent != ply then
			...
			if (ent:IsPlayer() and ent:Team() != ply:Team()) or IsValid(phys) then
...
function GM:PlayerDeathThink( pl )
	...
	if pl:Team() == TEAM_UNDEAD and (pl:GetPlayerClass() == CLASS_Z_BEHEMOTH or pl.SpawnAsClass == CLASS_Z_BEHEMOTH) 
		and (CurTime()-(pl.BehemothTimeout or 0) > 60 or (pl.BeheDeaths or 0) >= BEHEMOTH_DEATH_LIMIT) then
...
function Dissolve( ent )
	if ( IsValid( ent ) && !ent.Dissolving ) then            
		...
		dissolve:SetKeyValue( "dissolvetype", 1 )//0
	...
	if pl:Team() == TEAM_UNDEAD then
		...
		if pl.BabySpawn and pl.BabySpawn:IsValid() and pl.BabySpawn:GetTable():Alive() then
			timer.Simple(0.5,function ( ply ) -- destroy sacrifical baby spawn
				if IsValid(ply) and ply.BabySpawn:IsValid() and ply.BabySpawn:Alive() then
...
function GM:CanPlayerSuicide( ply )
	...
	return ALLOW_SUICIDE and (!(ply:Team() == TEAM_HUMAN and (team.NumPlayers(TEAM_HUMAN) == 1 or self:RoundTimeLeft() >= (ROUNDLENGTH - NOSUICIDE_TIME))) or ply:IsAdmin())
