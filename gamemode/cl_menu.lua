...
Options[11] = { Value = CL_HANDS, Desc = "Toggle custom hands", Cmd = "iw_clhands" }
...
function createMainFrame()
    ...
    frame:SetTitle( "Infected Wars Downfall" ) 
	...
	/*frame.PaintOver = function()
		draw.WordBox( 2, MENU_framewidth / 1.53, 0, "FREE UNLOCKS WEEKEND!", "ArialB_18", Color(160,35,35,(math.sin(RealTime() * 3) * 100) + 170), Color(255,255,255,255) )
	end*/
	...
	local buttonDonate = vgui.Create("DButton",frame)
	buttonDonate:SetText( "Donate")
	buttonDonate:SetPos(336,30)
	buttonDonate:SetSize(60,20)
	buttonDonate.DoClick = function( btn ) RunConsoleCommand("iw_menu_donate") end

	-- Create shop button
	local buttonShop = vgui.Create("DButton",frame)
	buttonShop:SetText( "Shop")
	buttonShop:SetPos(402,30)
...
function createScorePanel()
	subframe:SetTitle( "Achievements and score (achievements requires more than 3 players on server" )
...
function createOptionsPanel()
	...
	local songlist = vgui.Create("DComboBox", subframe)
...
--[[-----------------------------
	CREATE DONATE PANEL
-------------------------------]]
function createDonatePanel()
	subframe:SetTitle( "Donate!" )
	
	local box = vgui.Create("DTextEntry",subframe)
	box:SetPos( 10, 38 ) 
    box:SetSize( MENU_framewidth-60, MENU_subframeheigth-60 ) 
	box:SetEditable( false )
	box:SetMultiline( true )
	box:SetValue([[The Mr. Green servers are very active, and bandwidth doesn't pay for itself. We'll gladly accept donations!
	You can donate to the Mr. Green servers using PayPal! For every euro you donate you get 1000 Green-Coins 
	in return. 
	
	Green-Coins can be used to buy all sorts of things on our gameservers (not only for this server!). 
	They're stored in a global account. By linking your Steam ID to your Left4Green.com forum account, 
	you can manage your GC online.
	
	Visit MrGreenGaming.com and click "GreenCoins". For further instructions there's a link in the 
	footer of that page.]])
	
	local mrGreen = vgui.Create("DImage", subframe)
	mrGreen:SetPos(MENU_framewidth-40,38)
	mrGreen:SetSize(32,32)
	mrGreen:SetImage( "mrgreen/mrgreen" )
	mrGreen:SetImageColor( Color(255,255,255,255) )
	mrGreen:SizeToContents()
end 
...
function createShopPanel()
	...
	local donbutton = vgui.Create("DButton", subframe)
	donbutton:SetPos(405, 300)
	donbutton:SetSize(170, 28)
	...
	function itemDoClick(btn)
		...
		if (shopData[item].Cost == -1) then
			...
			itemcostlabel:SetText("COST: "..shopData[item].Cost.." GC")
		end
		...
		elseif (MySelf:GreenCoins() < shopData[item].Cost) then
...
function createAdminPanel()
	...
	--give exploit blocker
	local buttonEx = vgui.Create("DButton",subframe)
	buttonEx:SetText( "Get exploit blocker")
	buttonEx:SetPos(340,105)
	buttonEx:SetSize(65*2+5,25)
	...
	buttonEx.DoClick = function( btn )
		 RunConsoleCommand("say","!swep "..LocalPlayer():Name().." admin_exploitblocker")
	end 
...
function createClassPanel( start, team )	
	...
	local currentSelectedClass = 1
	
	-- On start, create a slightly different frame than the normal one
	if start then
		frame = vgui.Create("DFrame")
		frame:SetTitle( "Welcome to this Infected Wars Downfall server!" )
	...
	local HadFirstChoice = false
	if team == TEAM_HUMAN then
		for k = 1, #HumanClass do
			if HumanClass[k].Choosable then
				class[k] = classlist:AddChoice(HumanClass[k].Name .." (".. amountOfPlayerInClass(k,TEAM_HUMAN) .." players)", k, not HadFirstChoice)
				HadFirstChoice = true
			else
				class[k] = nil
			end
		...
		for k = 1, #UndeadClass do
			if UndeadClass[k].Choosable then
				class[k] = classlist:AddChoice(UndeadClass[k].Name .." (".. amountOfPlayerInClass(k,TEAM_UNDEAD) .." players)", k, not HadFirstChoice)
				HadFirstChoice = true
			else
				class[k] = nil
			end
	...
	--TODO: Fix updating of player counts
	--[[timer.Create("updatetimer", 1, 0, function()
		if not class[2]:IsValid() then
		...
		for k = 1, #class do
			if class[k] ~= nil then
				if team == TEAM_HUMAN then
					class[k]:SetText(HumanClass[k].Name.." ("..amountOfPlayerInClass(k,TEAM_HUMAN).." players)")
				else
					class[k]:SetText(UndeadClass[k].Name.." ("..amountOfPlayerInClass(k,TEAM_UNDEAD).." players)")
				end
			end
		end
	end)]]
	...
	local loadoutlist = vgui.Create("DComboBox",subframe)
	...
	function getSelectedClass(Id)
		...
		--[[ErrorNoHalt("getSelectedClass called in cl_menu. But it's not available.")
		...
		return 0]]
		if (currentSelectedClass == nil) then return 0 end
	...
	function classlist:OnSelect( index, value, data )
		currentSelectedClass = data
		--local selectedClass = getSelectedClass()
				if (team == TEAM_HUMAN) then
					numLoadouts = (#HumanClass[currentSelectedClass].SwepLoadout)
				elseif (team == TEAM_UNDEAD) then
					numLoadouts = (#UndeadClass[currentSelectedClass].SwepLoadout)
				end
				updateLoadoutList()

		print("Selected class ".. data)
	end
...
concommand.Add("iw_menu_donate", openDonateMenu) 
