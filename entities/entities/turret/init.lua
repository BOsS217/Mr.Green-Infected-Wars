...
function ENT:Initialize()
	...
	self.Entity:SetDTFloat( 0, 100 ) --health
	self.Entity:SetDTFloat( 1, 100 ) --maxhealth
	
	--self.Entity:SetNetworkedInt( "status", TurretStatus.active )
	self.Entity:SetDTInt( 0, TurretStatus.active ) --status
	
	self.kills = 0
	--self.Entity:SetNetworkedInt( "kills", self.kills )
	self.Entity:SetDTInt( 1, self.kills )
...
function ENT:SetHealthDefault( hp )
	self.Entity:SetDTFloat( 0, hp )
	self.Entity:SetDTFloat( 1, hp )
...
function ENT:Think()
	...
	if (not IsValid(self.Owner) or (self.Owner:IsPlayer() and not self.Owner:Alive())) and not self.Dead then
		self:SetOwner(nil)
	...
	--Shop upgrade
	if IsValid(self.Owner) and self.Owner:HasBought("searchengine") then
	...
	elseif self.attacking then
		...
		local distoen = 0
		if IsValid(self.enemytarget) then
			distoen = self.enemytarget:GetPos():Distance(self.Entity:GetPos())
		end
		
		if not IsValid(self.enemytarget) or not self.enemytarget:Alive() or self.lasttargetcontact+1.5 < CurTime() or distoen > self.loseengagedistance or 
...
function ENT:PhysicsUpdate( phys )
	if self:GetMode() == st.SHUTDOWN then
		return
	end
...
function ENT:Damage( amount, attacker )
	...
	self.Entity:SetDTFloat( 0, math.max(0,self.Entity:GetDTFloat(0) - amount) )
	...
	if self.Entity:GetDTFloat(0) <= 0 and not self.Dead then 
		...
		if IsValid(attacker) and attacker:IsPlayer() then
			...
			if IsValid(inflictor) then
				if string.find(inflictor:GetClass(),"hornetgun") then
					attacker:UnlockAchievement("quickfix")
				end
			end
		end
...
function ENT:SetMode(mode)
	self.status = mode
	self.Entity:SetDTInt( 2, self.status ) --mode
end
...
function ENT:AlignToAngle(angle)
	self.PhysObj:SetAngles(self:ApproachAngle( self.PhysObj:GetAngles(), angle, FrameTime()*self.turnspeed ))
...
function ENT:RestoreHealth( amount )
	...
	self.Entity:SetDTFloat( 0, math.min(self.Entity:GetDTFloat(1),self.Entity:GetDTFloat(0) + amount) )
...
function ENT:OnRemove()
	self.Entity:SetDTBool( 0, false )
	self.Entity:SetDTInt( 0, TurretStatus.destroyed )
end
