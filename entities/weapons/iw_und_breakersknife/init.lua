...
function SWEP:PrimaryAttack()
	...
	local bonus = math.Clamp(math.Clamp(self.Owner:Health()/self.Owner:GetMaximumHealth(),0,1)*self.BonusDamage,0,self.BonusDamage)
	...
	for k, pl in pairs(fucked_ones) do
		if (pl:IsPlayer()) then
			if (pl:Team() == TEAM_HUMAN) then
				...
				pl.ScreamEffect = CurTime() + fuckedlvl
